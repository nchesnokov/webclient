...
function onConnect(wsClient) {
  ...
  wsClient.send(JSON.stringify({ 'status': 'Connnected' }));
        ...
        case 'PUT':
          fs.writeFile(path.join(save_path, 'src', 'components', jsonMessage.name), jsonMessage.data, (err) => {
            ...
            wsClient.send(JSON.stringify({
              'id': jsonMessage.id, 'file': jsonMessage.name, 'stat': fs.statSync(path.join(save_path, 'src', 'components', jsonMessage.name))
            }))
          });
          break;
        case 'STAT':
          fs.stat(path.join(save_path, 'src', 'components', jsonMessage.name), jsonMessage.data, (error, stats) => {
            if (error) {
              console.log(error);
              wsClient.send(JSON.stringify({ 'id': jsonMessage.id, 'stats': {} }))
            }
            else {
              wsClient.send(JSON.stringify({ 'id': jsonMessage.id, 'stats': stats }));
            }
          });
          break;
        default:
