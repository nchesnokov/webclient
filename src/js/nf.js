const on_modify_models = (meta__cache__,dataForm,values) => {
    function _update(meta__cache__,diffs) {
    ...
    function _insert(meta__cache__,diffs) {
    ...
    function _delete(meta__cache__,diffs) {
    ...
    function _meta_update(meta__cache__,diffs) {
    ...
    function _m2m_remove(meta__cache__,diffs) {
        let row, c, idx;
    ...
    function _m2m_recursive_remove(meta__cache__,rows) {
        //let row,c,idx;
    ...
    function _o2m_remove(meta__cache__,diffs) {
        //let row,c,idx;
    ...
    function _o2m_recursive_remove(meta__cache__,rows) {
        //let row,c,idx;
        for (let i = 0, row, c, idx; i < rows.length; i++) {
            row = rows[i];
            c = meta__cache__.__containers__[row.__container__];
    ...
    function _m2m_append(meta__cache__,diffs) {
    ...
    function _o2m_append(meta__cache__,diffs) {
    ...
    function _apply_diffs(meta__cache__,diffs) {
        _m2m_remove(meta__cache__,diffs);
        _o2m_remove(meta__cache__,diffs)
        _m2m_append(meta__cache__,diffs);
        _o2m_append(meta__cache__,diffs)
        _update(meta__cache__,diffs);
        _insert(meta__cache__,diffs);
        _delete(meta__cache__,diffs);
        _meta_update(meta__cache__,diffs);
    }
    ...
    if ('__data__' in values) {
        ...
        _apply_diffs(meta__cache__,data);
    }
