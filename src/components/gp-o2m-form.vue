<template>
    <el-pagination v-if="cdata.length > 1" background layout="total, prev, pager, next, jumper"
        @current-change="handleCurrentChange" :page-size="pageSize" :total="cdata.length"></el-pagination>

    <el-form v-if="cdata.length > 0" :modelValue="cdata[page - 1].__data__" label-width="auto" status-icon
        inline-message>
        <el-form-item :label="colsLabel[col]" v-for="col in cols" :key="col">
            <el-input v-model="cdata[page - 1].__data__[col].name" @change="m2o_cache(cdata[page - 1], col)"
                v-if="['many2one', 'referenced'].indexOf(colsType[col]) >= 0"
                :prefix-icon="isCompute(col) ? 'el-icon-s-data' : ''" :readonly="readonly(col)">
                <template #suffix>
                    <el-button type="primary" size="small" :icon="Search"
                        @click="do_find(col, 'single', [], { 'item': cdata[page - 1] })"></el-button>
                    <el-button type="primary" size="small" :icon="DocumentAdd" @click="do_add(col)"></el-button>
                    <el-button v-if="cdata[page - 1].__data__[col].id != null" type="primary" size="small" :icon="Edit"
                        @click="do_edit(col, cdata[page - 1].__data__[col].id)"></el-button>
                    <el-button v-if="cdata[page - 1].__data__[col].id != null" type="primary" size="small" :icon="View"
                        @click="do_lookup(col, cdata[page - 1].__data__[col].id)"></el-button>
                </template>
            </el-input>
            <el-input v-model="cdata[page - 1].__data__[col].name" @change="related_cache(cdata[page - 1], col)"
                v-if="['related'].indexOf(colsType[col]) >= 0" :prefix-icon="isCompute(col) ? 'el-icon-s-data' : ''"
            ...
            <el-input v-model="cdata[page - 1].__data__[col]" @change="cache(cdata[page - 1], col)"
                        ...
                        <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item v-for="lang in $UserPreferences.langs" :key="lang.code"
                                    :command="{ col: col, lang: lang.code }">{{ lang.description }}</el-dropdown-item>
                            </el-dropdown-menu>
                        </template>
                    </el-dropdown>
                </template>
            ...
            <el-input v-model="cdata[page - 1].__data__[col]" @change="cache(cdata[page - 1], col)" autosize
                type="textarea" v-else-if="['text', 'xml'].indexOf(colsType[col]) >= 0"
                :prefix-icon="isCompute(col) ? Monitor : ''" :readonly="readonly(col)"></el-input>
            <el-date-picker :modelValue="cdata[page - 1].__data__[col]" @update:modelValue="cache($event)"
                v-else-if="colsType[col] == 'date'" :readonly="readonly(col)"></el-date-picker>
            <el-time-picker :modelValue="cdata[page - 1].__data__[col]" @update:modelValue="cache($event)"
                v-else-if="colsType[col] == 'time'" :readonly="readonly(col)"></el-time-picker>
            <el-date-picker :modelValue="cdata[page - 1].__data__[col]" @update:modelValue="cache($event)"
                type="datetime" v-else-if="colsType[col] == 'datetime'" :readonly="readonly(col)"></el-date-picker>
            <el-select :modelValue="cdata[page - 1].__data__[col]" @update:modelValue="cache($event)"
                v-else-if="colsType[col] == 'selection'" :disabled="readonly(col)">
            ...
            <gp-m2m-list :metas="metas" :model="metas[model].meta.columns[col].obj"
                :tableData="cdata[page - 1].__m2m_containers__[col]" v-else-if="colsType[col] == 'many2many'">{{
                        colsLabel[col]
                }}</gp-m2m-list>
            <el-checkbox :value="cdata[page - 1].__data__[col]" @update:modelValue="cache($event)"
                v-else-if="colsType[col] == 'boolean'" :disabled="readonly(col)"></el-checkbox>
        ...
        <el-tabs type="border-card" v-if="o2mcols.length > 0">
            <el-tab-pane :label="colsLabel[o2mcol]" v-for="o2mcol in o2mcols" :key="o2mcol">
                <gp-o2m-components :cid="cid" :metas="metas" :model="metas[model].meta.columns[o2mcol].obj"
                    :cdata="cdata[page - 1].__o2m_containers__[o2mcol]" :mode="mode"
