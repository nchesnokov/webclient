<template>
    ...
    <slot>
		...
		<el-form-item :label="t(colsLabel['shortname'])">
                <el-input
		...
		<el-form-item :label="t(colsLabel['isocode'])">
                <el-input
                    v-model="dataForm.__data__['isocode']"
                    :maxlength="colsSize['isocode']"
                    show-word-limit
                    @change="cache(dataForm, 'isocode')"
                    :readonly="readonly('isocode')"
                >
                    <template v-if="isCompute('isocode')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['isocode']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['isocode'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'isocode', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
