<template>
    ...
    <slot>
		...
		<el-form-item :label="t(colsLabel['module'])">
                <el-input
                    v-model="dataForm.__data__['module']"
                    :maxlength="colsSize['module']"
                    show-word-limit
                    @change="cache(dataForm, 'module')"
                    :readonly="readonly('module')"
                >
                    <template v-if="isCompute('module')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['module']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['module'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'module', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
		...
		<el-form-item :label="t(colsLabel['model'])">
                <el-input
                    v-model="dataForm.__data__['model']"
                    :maxlength="colsSize['model']"
                    show-word-limit
                    @change="cache(dataForm, 'model')"
                    :readonly="readonly('model')"
                >
                    <template v-if="isCompute('model')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['model']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['model'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'model', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
		...
		<el-form-item :label="t(colsLabel['rec_id'])">
                <el-input
                    v-model="dataForm.__data__['rec_id']"
                    @change="cache(dataForm, 'rec_id')"
                    :readonly="readonly('rec_id')"
                >
                    <template v-if="isCompute('rec_id')" #prefix>
                        <el-button type="primary" size="small" :icon="Monitor" />
                    </template>
                </el-input>

		</el-form-item>
		<el-form-item :label="t(colsLabel['file_id'])">
                <el-input
                    v-model="dataForm.__data__['file_id'].name"
                    @change="related_cache(dataForm, 'file_id', metas[model].meta.columns['file_id'].relatedy)"
                    :readonly="readonly('file_id')"
                >
                    <template v-if="isCompute('file_id')" #prefix>
                        <el-button type="primary" size="small" :icon="Monitor" />
                    </template>
                    <template #suffix>
                        <el-button type="primary" size="small" :icon="Search" @click="do_find('file_id')"></el-button>
                        <el-button
                            type="primary"
                            size="small"
                            :icon="DocumentAdd"
                            @click="do_add('file_id')"
                        ></el-button>
                        <el-button
                            v-if="dataForm.__data__['file_id'].id != null"
                            type="primary"
                            size="small"
                            :icon="Edit"
                            @click="do_edit('file_id', dataForm.__data__['file_id'].id)"
                        ></el-button>
                        <el-button
                            v-if="dataForm.__data__['file_id'].id != null"
                            type="primary"
                            size="small"
                            :icon="View"
                            @click="do_lookup('file_id', dataForm.__data__['file_id'].id)"
                        ></el-button>
                    </template>
                </el-input>				

		</el-form-item>
		<el-form-item :label="t(colsLabel['date_init'])">
                <el-date-picker
                    v-model="dataForm.__data__['date_init']"
                    type="datetime"
                    @change="cache(dataForm, 'date_init')"
                    :readonly="readonly('date_init')"
                ></el-date-picker>

		</el-form-item>
		<el-form-item :label="t(colsLabel['date_update'])">
                <el-date-picker
                    v-model="dataForm.__data__['date_update']"
                    type="datetime"
                    @change="cache(dataForm, 'date_update')"
                    :readonly="readonly('date_update')"
                ></el-date-picker>
