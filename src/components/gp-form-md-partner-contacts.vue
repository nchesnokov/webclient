<template>
    ...
    <slot>
		...
		<el-form-item :label="t(colsLabel['partner_id'])">
                <el-input
                    v-model="dataForm.__data__['partner_id'].name"
                    @change="m2o_cache(dataForm, 'partner_id')"
                    :readonly="readonly('partner_id')"
                >
                    <template v-if="isCompute('partner_id')" #prefix>
                    ...
                    <template #suffix>
                        <el-button
                            ...
                            size="small"
                            :icon="Search"
                            @click="do_find('partner_id', 'single', [], { 'item': dataForm })"
                        ></el-button>
                        <el-button
                            type="primary"
                            size="small"
                            :icon="DocumentAdd"
                            @click="do_add('partner_id')"
                        ></el-button>
                        <el-button
                            v-if="dataForm.__data__['partner_id'].id != null"
                            type="primary"
                            size="small"
                            :icon="Edit"
                            @click="do_edit('partner_id', dataForm.__data__['partner_id'].id)"
                        ></el-button>
                        <el-button
                            v-if="dataForm.__data__['partner_id'].id != null"
                            type="primary"
                            size="small"
                            :icon="View"
                            @click="do_lookup('partner_id', dataForm.__data__['partner_id'].id)"
                        ></el-button>
                    </template>
                </el-input>					

		</el-form-item>
		<el-form-item :label="t(colsLabel['usage'])">
                <el-select
		...
		<el-form-item :label="t(colsLabel['sequence'])">
                <el-input
                    v-model="dataForm.__data__['sequence']"
                    @change="cache(dataForm, 'sequence')"
                    :readonly="readonly('sequence')"
                >
                    <template v-if="isCompute('sequence')" #prefix>
                        <el-button type="primary" size="small" :icon="Monitor" />
                    </template>
                </el-input>

		</el-form-item>
		<el-form-item :label="t(colsLabel['firstname'])">
                <el-input
		...
		<el-form-item :label="t(colsLabel['lastname'])">
                <el-input
		...
		<el-form-item :label="t(colsLabel['middlename'])">
                <el-input
                    v-model="dataForm.__data__['middlename']"
                    :maxlength="colsSize['middlename']"
                    show-word-limit
                    @change="cache(dataForm, 'middlename')"
                    :readonly="readonly('middlename')"
                >
                    <template v-if="isCompute('middlename')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['middlename']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['middlename'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'middlename', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
		...
		<el-form-item :label="t(colsLabel['fullname'])">
                <el-input
		...
		<el-form-item :label="t(colsLabel['gender'])">
                <el-select
                    v-model="dataForm.__data__['gender']"
                    @change="cache(dataForm, 'gender')"
                    :disabled="readonly('gender')"
                >
                    <el-option
                        v-for="item in selOptions['gender']"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                    ></el-option>
		...
		<el-form-item :label="t(colsLabel['marital'])">
                <el-select
                    v-model="dataForm.__data__['marital']"
                    @change="cache(dataForm, 'marital')"
                    :disabled="readonly('marital')"
                >
                    <el-option
                        v-for="item in selOptions['marital']"
                        :key="item.value"
                        :label="item.label"
                        :value="item.value"
                    ></el-option>
		...
		<el-form-item :label="t(colsLabel['birthday'])">
                <el-date-picker
                    v-model="dataForm.__data__['birthday']"
                    @change="cache(dataForm, 'birthday')"
		...
		<el-form-item :label="t(colsLabel['image_medium'])">
		...
		<el-form-item :label="t(colsLabel['image_small'])">
		...
		<el-form-item :label="t(colsLabel['address_id'])">
                <el-input
                    v-model="dataForm.__data__['address_id'].name"
                    @change="m2o_cache(dataForm, 'address_id')"
                    :readonly="readonly('address_id')"
                >
                    <template v-if="isCompute('address_id')" #prefix>
                        <el-button type="primary" size="mini" icon="el-icon-s-data" />
                    </template>
                    <template #suffix>
                        <el-button
                            ...
                            size="small"
                            :icon="Search"
                            @click="do_find('address_id', 'single', [], { 'item': dataForm })"
                        ></el-button>
                        <el-button
                            type="primary"
                            size="small"
                            :icon="DocumentAdd"
                            @click="do_add('address_id')"
                        ></el-button>
                        <el-button
                            v-if="dataForm.__data__['address_id'].id != null"
                            type="primary"
                            size="small"
                            :icon="Edit"
                            @click="do_edit('address_id', dataForm.__data__['address_id'].id)"
                        ></el-button>
                        <el-button
                            v-if="dataForm.__data__['address_id'].id != null"
                            type="primary"
                            size="small"
                            :icon="View"
                            @click="do_lookup('address_id', dataForm.__data__['address_id'].id)"
                        ></el-button>
                    </template>
                </el-input>					

		</el-form-item>
		<el-form-item :label="t(colsLabel['work_phone'])">
                <el-input
                    v-model="dataForm.__data__['work_phone']"
                    :maxlength="colsSize['work_phone']"
                    show-word-limit
                    @change="cache(dataForm, 'work_phone')"
                    :readonly="readonly('work_phone')"
                >
                    <template v-if="isCompute('work_phone')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['work_phone']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['work_phone'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'work_phone', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
		...
		<el-form-item :label="t(colsLabel['mobile_phone'])">
                <el-input
                    v-model="dataForm.__data__['mobile_phone']"
                    :maxlength="colsSize['mobile_phone']"
                    show-word-limit
                    @change="cache(dataForm, 'mobile_phone')"
                    :readonly="readonly('mobile_phone')"
                >
                    <template v-if="isCompute('mobile_phone')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['mobile_phone']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['mobile_phone'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'mobile_phone', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
		...
		<el-form-item :label="t(colsLabel['work_email'])">
                <el-input
                    v-model="dataForm.__data__['work_email']"
                    :maxlength="colsSize['work_email']"
                    show-word-limit
                    @change="cache(dataForm, 'work_email')"
                    :readonly="readonly('work_email')"
                >
                    <template v-if="isCompute('work_email')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['work_email']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['work_email'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'work_email', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
		...
		<el-form-item :label="t(colsLabel['work_location'])">
                <el-input
                    v-model="dataForm.__data__['work_location']"
                    :maxlength="colsSize['work_location']"
                    show-word-limit
                    @change="cache(dataForm, 'work_location')"
                    :readonly="readonly('work_location')"
                >
                    <template v-if="isCompute('work_location')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['work_location']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['work_location'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'work_location', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
...
export default defineComponent({
    name: 'gp-form-md-partner-contacts',
})
