<template>
    ...
    <slot>
		...
		<el-form-item :label="t(colsLabel['name2'])">
                <el-input
                    v-model="dataForm.__data__['name2']"
                    :maxlength="colsSize['name2']"
                    show-word-limit
                    @change="cache(dataForm, 'name2')"
                    :readonly="readonly('name2')"
                >
                    <template v-if="isCompute('name2')" #prepend>
                        <el-button type="primary" size="small" :icon="Monitor" />
                        <el-dropdown v-if="colsTranslate['name2']" @command="i18nCommand">
                            <span class="el-dropdown-link">
                                {{ colsLang['name2'].toLowerCase() }}
                                <i
                                    class="el-icon-arrow-down el-icon--right"
                                ></i>
                            </span>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item
                                        v-for="lang in $UserPreferences.langs"
                                        :key="lang.code"
                                        :command="{ col: 'name2', lang: lang.code }"
                                    >{{ lang.description }}</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
