  ...
  export default {
    ...
    methods: {
	  onLoadMeta: function(msg){
		//console.log('on-load-meta-schedule:',msg);
		this.fields = Object.keys(this.view.columns);
		if (this.fields.indexOf(this.meta.names.from_date) == -1) this.fields.push(this.meta.names.start_date);
		if (this.fields.indexOf(this.meta.names.to_date) == -1) this.fields.push(this.meta.names.end_date);
		//this.send('on_schedule',['models',this.model,'select',{'fields':this.fields}]);

		},
	  on_schedule: function(rows){
		//console.log('rows-schedule:',rows);
		//this.events.splice(0,this.events.length);
		
		if (rows.length > 0) {
			//this._buildEvents(rows);
			}
		},
