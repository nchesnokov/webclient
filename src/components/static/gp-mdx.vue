...
import { onBeforeMount, ref, reactive, watch, getCurrentInstance } from "vue";
...
onBeforeMount(async () => {
  ...
  vfields.splice(
    0,
    vfields.length,
    ...props.metas[props.model].views.mdx.columns.map((v) => v.col)
  );
  ...
  for (let i = 0; i < vfields.length; i++) {
    switch (props.metas[props.model].meta.columns[vfields[i]].type) {
      case "char":
      case "varchar":
        if (vfields[i] == props.metas[props.model].meta.names.rec_name)
          colFields.push({
      ...
      case "related":
        rowFields.push({
          label: props.metas[props.model].meta.columns[vfields[i]].label,
          getter: new Function("item", "return item." + vfields[i] + ".name"),
        });
        break;
      case "selection":
        selectionFromForm[vfields[i]] = {};
        selections =
          props.metas[props.model].meta.columns[vfields[i]].selections;
