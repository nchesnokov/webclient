...
<template>
    ...
    <gp-selectable v-if="showSearch" :columns="metas[model].meta.columns" :names="metas[model].meta.names" :cid="cid"
        @update:search="do_search" @update:search-cancel="showSearch = false">
    </gp-selectable>
    ...
    <el-container>
        <el-table :data="tableData" row-key="id" border default-expand-all lazy :load="load" :tree-props="treeProps"
            style="width: 100%">
            <el-table-column :prop="getProp(col)" :label="metas[model].meta.columns[col].label"
                v-for="col in metas[this.model].views.tree.columns.map((v) => v.col).filter(col => col != metas[model].meta.names.childs_id)"
                :key="col" fit />
        </el-table>
    </el-container>
