...
const onSelect = () => {
  if (
    ["find", "autofind"].indexOf(props.callbackOpts.mode) >= 0 &&
    props.callback != null
  ) {
    if (props.mode == "single")
      ...
      let vals = [];
      for (
        let i = 0, rec_name = metas[props.model].meta.names.rec_name;
...
const on_load_meta = (msg) => {
  ...
  if (props.callbackOpts.mode == "autofind")
    do_search({ cond: [], offset: 0, limit: 80 });
};
