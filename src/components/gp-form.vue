...
<slot>
    ...
    <el-form v-if="'__data__' in dataForm && Object.keys(dataForm.__data__).length > 0" :model="dataForm.__data__" label-width="auto">
        <el-form-item :label="colsLabel[col]" v-for="col in cols" :key="col">
            ...
            <el-input v-model="dataForm.__data__[col]" maxlength="10" show-word-limit v-else-if="['char','varchar','composite'].indexOf(colsType[col]) >= 0" :prefix-icon="isCompute(col) ? 'el-icon-s-data':''" :readonly="readonly(col)">
                <template #prefix>
            ...
            <el-input v-model="dataForm.__data__[col]" v-else-if="['uuid','integer','float','decimal','numeric','timedelta'].indexOf(colsType[col]) >= 0" :prefix-icon="isCompute(col) ? 'el-icon-s-data':''" :readonly="readonly(col)"></el-input>
            <el-input v-model="dataForm.__data__[col]" autosize type="textarea" v-else-if="['text','xml'].indexOf(colsType[col]) >= 0" :readonly="readonly(col)">            
            </el-input>
