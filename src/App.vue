  ...
  <el-container>
    ...
    <el-footer>
      {{
        ...
        " " +
        timestampLogged
        : "&copy; GSRP5 " + new Date().getFullYear()
      }}
    </el-footer>
  </el-container>
</template>
...
import { WebSocketClient,default_config } from './js/wspromisify';
...
// root.appContext.app.config.globalProperties.$ws = new WebSocketClient(default_config,{lazy:false})  
...
const handleUserMenuCommand = async (command) => {
  switch (command) {
    case "preferences":
      ...
      break;this.ws.send(this.config.encode(id, msg, this.config.server))
    case "logout":
      await proxy.$ws.send([cid.value, "logout", {}]);
...
const do_login = async (event) => {
  if (event === "cancel") isLoginFormShow.value = false;
    ...
    root.appContext.app.config.globalProperties.$ws = new WebSocketClient(default_config,{lazy:false,on_close_websocket:on_close_websocket})
      if (!proxy.$ws.config.lazy) await proxy.$ws.ready()
      // else {
      //   proxy.$ws.connect()
      //   await proxy.$ws.ready()
      // } 
    Object.assign(cinfo, event);
        let r = await proxy.$ws.send([
...
const do_user_preferences = async (event) => {
  ...
  let records = {
    ...
    country: proxy.$UserPreferences.country_names[event.couthis.config.on_close_websocketntry],
  ...
  await proxy.$ws.send([cid.value, "_commit", {}]);
...
const MyloadModule = async (name, names) => {
  ...
  if (typeof stat === "object" && "stats" in stat) {
    if (name in names) {
      ...
      const view = await proxy.$ws.send([
        cid.value,
        "models",
        "bc.ui.model.views",
        "getSFC",
